<script setup>
  ...
  async function closeModal() {
    await nuxtApp.callHook("modal-close", {})
  }
</script>

<template>
  <div class="modal__bg">
    <div class="modal__container" v-on-click-outside="closeModal">
      <div class="modal__header">
        <div class="modal__img">
          <utils-button :size="2" icon="x" :icon_size="26" :icon_stroke="1.8" @click="closeModal" />
      ...
      <div class="modal__body">
        ...
        <div class="flex items-center">
          <h3>{{ useStringFormat().formatPrice(4000) }}</h3>
          <h4 class="ml-1">per month</h4>
        </div>
        <h5>BALANCE</h5>
        <div class="flex items-center">
          <h4 class="text-brand-light-gray">Remaining</h4>
          <h3 class="ml-2">{{ useStringFormat().formatPrice(1551.88) }}</h3>
          <h4 class="ml-auto text-brand-light-gray">Spent</h4>
          <h3 class="ml-2">{{ useStringFormat().formatPrice(2448.12) }}</h3>
        </div>
        <div />
