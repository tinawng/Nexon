<template>
  <div class="data_table__container">
    ...
    <div
      ...
      "
    >
      <h4>{{ formatHour(data.date) }}</h4>
      <div class="flex">
        <img
          class="w-12 mr-4 rounded-full"
          :src="data.vendor_icon"
          alt="vendor-icon"
        />
        <div class="flex flex-col justify-evenly">
          <span class="font-medium">{{ data.vendor }}</span>
          <h4 class="flex">
            {{ data.vendor_type }} • {{ data.author }} • {{ data.departement }}
          </h4>
        </div>
      </div>
      <div class="flex">
        <Icon class="mr-4" variant="file" /><Icon variant="message" />
      </div>
      <div class="text-right">
        {{ formatPrice(data.amount) }}
        <h4 class="flex justify-end items-center">
          <Icon class="mr-2" variant="log-in" :size="16" />
          {{ formatPrice(data.cashback) }}
        </h4>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    selectRow(index) {
      ...
      this.$refs[`table-row-${index}`][0].classList.add(
